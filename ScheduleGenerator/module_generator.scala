...
object ModuleGenerator {
  ...
  def handleNodeLabels(node: Node): String = node.head.label match {
    ...
    case "MemoryRegions" => {
      ...
      val regions =
        node.child.filter(child => this.checkAttributeValidity("Type", child));
    ...
    case "PartitionPorts" => {
      ...
      var partitionPorts: Seq[Node] =
        node.child.filter(child => child.head.label == "PartitionPort");
    ...
    case "HealthMonitoring" => {
      ...
      var multiPartitions =
        node.child.filter(child => child.head.label == "MultiPartitionHM");
      emitString = emitString +
      ...
      var moduleHms =
        node.child.filter(child => child.head.label == "ModuleHM");
      emitString = emitString +
      ...
      var partitionsHM =
        node.child.filter(child => child.head.label == "PartitionHM");
      emitString = emitString +
  ...
  def generatePartitionPorts(nodes: Seq[Node], isSampling: Boolean): String =
    nodes match {
  ...
  def retrieveNodeAttributeString(node: Node, attr: String): String =
    attr match {
      case x if this.checkAttributeValidity(x, node) =>
        node.attribute(attr).get.toString;
  ...
  def emitIncludes(incls: List[String]): String = incls match {
    case x :: xs if xs != Nil =>
      this.emit(f"#include ${this.sInclude(x)} \n") + this.emitIncludes(xs)
